  ...
  bottom.contact.parameters = function( bcp=list() ) {
    ...
    if (bcp$datasource =="groundfish") {
      ...
      if ( bcp$nr < 3500 & bcp$nr >= 2000 ) {
        ...
        bcp$noisefilter.target.r2.final = 0.7
      } 

      if ( bcp$nr < 2000 & bcp$nr >= 1200 ) {
        bcp$noisefilter.target.r2 = 0.7
    ...
    if ( !exists("noisefilter.target.r2.final", bcp)) bcp$noisefilter.target.r2.final = 0.6 # for noise filtering  .. ignore variations less than this threshold
    ...
    if ( !exists("smooth.target.r2", bcp)) bcp$smooth.target.r2 = 0.6  # for smooth of dZ (slopes)
