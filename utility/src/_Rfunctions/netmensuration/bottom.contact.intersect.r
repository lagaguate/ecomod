...
bottom.contact.intersect = function( sm, tdif.min, tdif.max )  {
  ...
  duration = as.numeric( difftime(  sm$timestamp[i1], sm$timestamp[i0] , units="mins" ) )
  if ( duration > tdif.min & duration < tdif.max ) res =  c( sm$timestamp[i0], sm$timestamp[i1] )
