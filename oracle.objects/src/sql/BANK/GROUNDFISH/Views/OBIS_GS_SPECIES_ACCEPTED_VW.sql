--------------------------------------------------------
--  DDL for View OBIS_GS_SPECIES_ACCEPTED_VW
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "GROUNDFISH"."OBIS_GS_SPECIES_ACCEPTED_VW" ("GIVEN_SPEC_CODE", "GIVEN_COMMON_NAME", "GIVEN_SCIENT_NAME", "FAO_CODE", "FAO_E_COMMON_NAME", "FAO_F_COMMON_NAME", "GIVEN_TSN", "ACCEPTED_TSN", "UNACCEPT_REASON") AS 
  (
...
(
select ost.* from
obis_gs_species_tsn_vw ost
INNER JOIN
-- ---- Current design forces scientificnames with multiple species codes to be "marked" for
-- ---- manual review ... thus cnt_scient
(
select given_scient_name, count(given_spec_code) cnt_scient, count(given_tsn) cnt_gvn, count(distinct accepted_tsn) cnt_accpt 
from obis_gs_species_tsn_vw
