--------------------------------------------------------
--  DDL for View OBIS_GROUNDFISH_2
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "GROUNDFISH"."OBIS_GROUNDFISH_2" ("COLLECTIONCODE", "CATALOGNUMBER", "FIELDNUMBER", "COLLECTOR", "SDATE", "ETIME", "LOCALITY", "STARTLATITUDE", "STARTLONGITUDE", "ENDLATITUDE", "ENDLONGITUDE", "LATITUDE", "LONGITUDE", "MINIMUMDEPTH", "MAXIMUMDEPTH", "TEMPERATURE", "SAMPLESIZE", "OBSERVEDWEIGHT", "OBSERVEDWEIGHT2", "SAMPWGT", "TOTWGT", "OBSERVEDINDIVIDUALCOUNT", "GEAR", "GEARDESCRIPTION", "MISSION", "SETNO", "SPECIES", "MINIMUMSIZE", "MAXIMUMSIZE", "AVERAGESIZE") AS 
...
--    'PopulatiON Ecology Division, DFO Maritimes Industry Surveys Data Base, '||TO_CHAR(sysdate,'YYYY-MM-DD')||', Bedford Institute of Oceanography' AS Citation, 
	   ...
	   NVL(decode(a.sampwgt, 0.0, a.totwgt, a.sampwgt),0.0) AS ObservedWeight,
     decode(a.sampwgt, 0, a.totwgt, NULL, a.totwgt, a.sampwgt) AS ObservedWeight2,
     a.sampwgt, a.totwgt,
--
