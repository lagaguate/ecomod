--------------------------------------------------------
--  DDL for View OBIS_GS_SPECIES_TSN_VW
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "GROUNDFISH"."OBIS_GS_SPECIES_TSN_VW" ("GIVEN_SCIENT_NAME", "GIVEN_SPEC_CODE", "GIVEN_TSN", "ACCEPTED_TSN", "UNACCEPT_REASON") AS 
  (
...
CASE
  ...
  ELSE ltrim(rtrim(given_scient_name))
END givenname1,
CASE
  WHEN instr(given_scient_name, ' ') > 0
  THEN substr(given_scient_name, instr(given_scient_name, ' ')+1, length(given_scient_name))
  ELSE null
END givenname2 
