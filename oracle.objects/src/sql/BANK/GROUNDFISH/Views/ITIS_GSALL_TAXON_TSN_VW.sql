--------------------------------------------------------
--  DDL for View ITIS_GSALL_TAXON_TSN_VW
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "GROUNDFISH"."ITIS_GSALL_TAXON_TSN_VW" ("GIVEN_SCIENT_NAME", "GIVEN_SPEC_CODE", "GIVEN_TSN", "ACCEPTED_TSN", "UNACCEPT_REASON") AS 
  (
...
CASE
  ...
  THEN substr(given_scient_name, 1, instr(given_scient_name, ' ')-1)
  ELSE trim(given_scient_name)
END givenname1,
CASE
  WHEN instr(given_scient_name, ' ') > 0
  THEN trim(substr(given_scient_name, instr(given_scient_name, ' ')+1, length(given_scient_name)))
  ELSE null
END givenname2 
from GSALL_SPECIES_CODES_USED_VW) o
INNER JOIN
...
ON upper(o.givenname1) = upper(i.unit_name1) and i.unit_name3 is null and i.unit_name4 is null 
...
upper(o.givenname2) IN('SP.','F.','O.','S.O.','C.','P','S.F.','S.C.','SPP','S.','P.','SO.','SP','SPP.','S.P.')   )))
LEFT OUTER JOIN
