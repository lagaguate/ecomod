  ...
  sizespectrum.db = function( DS="", p=NULL, nss.taxa=NULL, nss.type=NULL, nss.base=NULL ) {
    ...
    if (DS %in% c("sizespectrum", "sizespectrum.redo") ) {
      ...
      sm$t = habitat.lookup.simple( sm,  p=p, vnames="temp", lookuptype="temperature.weekly", sp.br=p$interpolation.distances ) 
    ...
    if (DS %in% c("sizespectrum.by.set", "sizespectrum.by.set.redo") ) {
      ...
      if (DS == "sizespectrum.by.set" ) {
        fn = file.path( ddir, paste(  "size.spectrum", nss.taxa, nss.type, nss.base, sep="." ) )
        load( fn )
        return (ss )
      }
      ...
      x$spec = taxa.specid.correct( x$spec, method=tx )
    ...
    if (DS %in% c( "sizespectrum.collapse.yr", "sizespectrum.collapse.yr.redo") ) {
      ...
      ss = sizespectrum.db( "sizespectrum.by.set", nss.taxa=nss.taxa[1], nss.type=nss.type, nss.base=nss.base )
      ...
      for (tx in nss.taxa) {
        ss = sizespectrum.db( "sizespectrum.by.set", nss.taxa=tx, nss.type=nss.type, nss.base=nss.base ) 
