...
diet.analysis<-function() {
	...
	options(warn=-1)
channel =   odbcConnect("oracle.user.server","oracle.user.user","oracle.user.password") #called globally
    ...
    cat("Please report bugs to: Adam.Cook@dfo-mpo.gc.ca\n")
    cat("---------------------------------------------------------- \n")
    cat("\n")
...
if(choose ==4) { #mean diets
			...
			if(ad=='y' || ad=='yes') ah<-1
       		...
       		diet.data$mean.diets<-mean.diet(diet.data=diet.data$raw.diet.data,prey.grouping=pp,remove.singletons=ah,remove.influentials=ag,percent.diff=as.numeric(af))
       		 ...
       		 if (ab == "yes" || ab == "y") {
       		 ll<-menu(c('5','10','20','Cutpoint','Slot'),title='Choose a Length grouping:')
       				if(ll==4) {
       				cat("What is your cutpoint length <= ? \n")
       				l1<-scan(what = "", nlines = 1, quiet = TRUE)
              ...
              if(ll==5) {
              cat("What is your slot lengths x y? \n")
              l1<-scan(what = "", nlines = 1, quiet = TRUE)
              diet.data$mean.diets<-mean.diet.by.length(data=diet.data$raw.diet,prey.grouping=pp,remove.singletons=ah,remove.influentials=ag,percent.diff=as.numeric(af),slot=T,lengths=as.numeric(l1))              
              }
       				else {
