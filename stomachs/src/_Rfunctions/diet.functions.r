#source("C:\\Documents and Settings\\cooka\\Desktop\\Scripts\\Food habits scripts\\diet analysis\\diet.functions.R")
data.src<-function() {
...
mean.diet.by.length <- function(data,lengths, cut.p=F, prey.grouping,remove.singletons,remove.influentials,percent.diff,slot=F) {
	mult.windows()
	if(cut.p) {
	...
	if(slot) {
		   data = data[which(data$FLEN>=lengths[1] & data$FLEN<=lengths[2]),]
		   a<-mean.diet(data,prey.grouping=prey.grouping,remove.singletons=remove.singletons,remove.influentials=remove.influentials,percent.diff=percent.diff,by.lengths=F)
		}
	if(lengths %in% c(1,2,3) & cut.p==FALSE & slot==FALSE) {
		if(lengths==1) data$RFLEN <- round.flens(data$FLEN,grp=5) #5cm
		if(lengths==2) data$RFLEN <- round.flens(data$FLEN,grp=10) #10cm
...
###################
#####choose 6  for stratified numbers per tow
##################
...
save.to.csv<-function(data) {
	...
	if(!is.data.frame(data)){
		for(i in 1:length(data)) {
		 ...
		 nna<-sqlQuery(channel,paste("select distinct fam, pretty_f_names from prey_spec_details where fam in ('",aaf,"');",sep=""))
	 	if(nrow(nna)>1) data$Prey.names<-nna[match(data$Prey,nna$FAM),2]	
...
#make a list of jack knifed data
...
#stratified diet estimates
#estimate mean diet per length group(from talley sheet) for each set as well as the variance (if only one fish sampled variance is 0)
#estimate mean diet per strata for each length group as wells as variance estimated as per warren et al 1994
#estimate mean clen per stata for each length group as well as variance as per warren 1994
#if combining lengths do this within strata- weight the diet estimates by the biomass of each length group sampled as well as the variances
#estimate the mean and variance for the stratified number of fish use the total biomass of the fish 
