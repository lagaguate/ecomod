marfissci.simple.map<-function(df, 
  ...
  classes = classIntervals(df.sp@data[,c(colour.by)], n=5, style= "quantile", dataPrecision=0)
  ...
  df.sp@data = merge(df.sp@data,unique(color.df), all.x = T)
  df.sp@data = df.sp@data[order(df.sp@data$ORD),]
  df.pr = spTransform(df.sp, CRS(crs.out), match.ID=F)
  ...
  points(df.pr, col = df.pr@data$colcode, pch = 22, cex = 0.2)
