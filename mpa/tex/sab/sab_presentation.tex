  ...
  \frametitle{Productivity}
    ...
    \begin{columns}[c]
      \begin{column}{.5\textwidth}
        \begin{itemize}
          \item Complex idea: space, time, organisational components
          \item Production is not the same as standing biomass 
...
\begin{frame}[shrink]
  ...
  \begin{itemize}
    ...
    \item Extra complex idea: space, time, organisational components $\bigotimes$~$H$
    \item Two approaches:
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item due to natural complexity, such models will always necessarily be incomplete
      \end{itemize}
      \item \textbf{Integrative} 
      \begin{itemize}
        \item the biota living in a given time and location fully integrates of all relevant environmental and biological factors at their proper space time scales, including the cumulative effects
        \item characterising taxonomic composition will estimate "habitat space"
      \end{itemize}
    \end{itemize}
    \item Estimate dynamics via a modeled solution to logistic state space model
  \end{itemize}
\end{frame}
...
\begin{frame}[shrink]
  \frametitle{Functional-habitat}
    \begin{columns}[c]
      \begin{column}{.4\textwidth}
        \begin{itemize}  
          \item Bernoulli process $\sim$ environmental factors/gradients
          \item Problem: how do we put the egg back together again
        \end{itemize}
      \end{column}
      \begin{column}{.6\textwidth}
        \begin{figure}[h]
...
\begin{frame}
  ...
  \begin{itemize}  
    \item Organism community indicates/reflects the type of habitat ($\equiv$~all biological and environmental interactions, short and long time and space scales, instantaneous and cumulative)
    \item Metric: Standard multivariate methods of data ordination. 
    \begin{itemize}
      \item Principal Components Analysis -- correlational structure
      \item Correspondence Analysis -- Chi-squared differences
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Spatial scale will determine which processes will be relevant
    \begin{itemize}
      \item If spatial variations in the productivity of a species of interest is small relative to the size of an MPA, the chances of the MPA having an influence upon the species is enhanced. This is usually the case when short-range processes dominate (e.g., less mobile species, weakly dispersing, low currents, habitat heterogeneity at small scales). 
      \item If spatial scale is larger than the MPA, then it would mean that broader/larger processes were influencing the productivity of the species (e.g., higher mobility or dispersal processes/current, and stronger spatial connectivity, habitat heterogeneity at larger scales) -- resulting in a lower likelihood of the MPA having an influence upon the species or components of interest.
      ...
      \item Measure via semivariogram
    \end{itemize}
  \end{itemize}
\end{frame}
    ...
    \begin{columns}[c]
      \begin{column}{.4\textwidth}
        ...
        \end{itemize}
          ...
          DEFINE: Spatial scale = distance at which the variance increases asymptotically to $\sim~0.75\%$ of the total variance.
        \end{column}
        \begin{column}{.6\textwidth}
         \begin{figure}
           \centering
...
\begin{frame}
  ...
  \begin{columns}[c]
    \begin{column}{.4\textwidth}
      \begin{itemize}
        \item Temporal stochastic process, $y$ defined at some time $t$
    ...
    \begin{column}{.6\textwidth}
      \begin{figure}
        \centering
        \includegraphics[width=0.5\textwidth]{\ecomod mpa/matern.jpg}
        \caption{Different expectations of cumulative variance as a function of time.}
        \label{fig:matern}
      \end{figure}
    \end{column}
  \end{columns}    
...
\begin{itemize}
  ...
  \item Recent methods, using a Markov Random Field representation of $y$ permits some interesting classes of space-time models to be parameterized. 
  \item Another class of models known as Stochastic spatio-temporal simulation models, also known as birth-death models show promise.  
...
\begin{frame}
  ...
  \begin{itemize} 
    ...
    \item genetic similarity (no work) .. Data gap
  \end{itemize}
...
\begin{frame}
  ...
  Focus upon the phenomenolgoical approach.
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Describe system state, $y$ (e.g., biomass, biodiversity, habitat, etc) as a logistic equation
    \item 
  \end{itemize}
 
Use on aggregate estimates of biomass (productivity), biodiversity (biodiversity change), habitat (habitat change) and other indicators of interest
The discrete form the basic normalised logistic equation is:
...
with parameters, $\theta=\{r,K\}$

A state space representation where an additional observation model:
...
Question: How to express the cumulative impacts on productivity, habitat, biodiversity, and endangered or threatened species    
Way forward: normalise each threat on a zero-one scale to compare the intensity of threats across the region 
weighted and combined to examine cumulative anthropogenic threats 
