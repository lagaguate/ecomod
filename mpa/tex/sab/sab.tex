...
\section{Data} 
In this section we focus upon a description of the data chosen for inclusion in this assessment. The purpose of the section is basic: to clearly identify the data, sampling design and the associated assumptions and methods required/used to filter and integrate them in an informative manner. For the sake of transparency, all data assimilation and QA/QC methods have been encoded in an open-sourced analytical environment R \cite{rCran} and made publicly accessible and revision controlled via \url{https://github.com/jae0/ecomod/}. As such, we try to ensure that this work is a real and flexible \textbf{structural framework} from which a common and vetted data system can be used coherently by multiple parties to develop further methods and approaches. 
...
\begin{figure}[h]
  ...
  \centering
  \includegraphics[width=0.65\textwidth]{\ecomod temperature/maps/SSE/bottom.predictions/global/{temperatures.bottom}.png}
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.6\textwidth]{\ecomod mpa/analysis/toweddistance.pdf}
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.6\textwidth]{\ecomod mpa/analysis/{wing.v.door}.pdf}
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.6\textwidth]{\ecomod mpa/analysis/{sa.comparison}.pdf}
...
\begin{figure}
  \label{fig:groundfishSweptAreaAll}
  \centering
  \includegraphics[width=0.6\textwidth]{\ecomod mpa/analysis/{sa.comparison.all}.pdf}
  \caption{Surface area estimates based on GSINF logged start-end positions vs computed surface area estimated from tow track and net configuration as well as modelled solutions.}
\end{figure}
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.6\textwidth]{\ecomod mpa/analysis/numberOfSets.pdf}
...
\begin{itemize}
  \item Deconstructive -- make increasing more precise habitat definitions by adding more environmental and biological factors for increasingly more precise categories of organisms   
  \item Integrative -- the biota living in a given time and location represent the true integration of all interacting environmental and biological factors and so in effect characterises the full system-level concept of "habitat space" 
\end{itemize}


\subsubsection{Deconstructive-habitat modeling}
\label{sec:habitatSpecies}

An immediately evident way of describing \textbf{Deconstructive-habitat} space of an organism is to examine the presence-absence or relative abundance of organisms as a function of environmental gradients and biological/life history constraints. From such information, the likelihood of a given location to be potential habitat for an organism of interest can be derived. The simplest method is to develop a probability model under the assumption that the presence or absence of an organism is a Bernoulli process. This can be readily parameterised using standard Generalized Linear Models. However, as environmental constraints are almost always modal in influence given a wide enough environmental gradient, a nonlinear model is more useful. Generalized Additive Models and Autoregressive Models are two methods that can deal with these environmental constraints in a simple and efficient manner. The utility of such an approach is most relevant for organisms with highly specific habitat requirements. They are used in this framework.

The intent is to develop such Deconstructive habitat models for key species of interest: wolffish, cod, etc., to assess changes in their available "habitat". Though the Deconstructive-habitat concept itself does not exclude species interactions, in actual practice, they are generally ignored as they make the statistical estimation impossibly over-parameterized (but see \cite{snowcrabResdoc} for one possible solution). Further, as there will always be factors that are either poorly known, poorly sampled, or poorly parameterized (e.g., dissolved oxygen, pH, redox, bacteria, jellyfish, squid, pollution, substrate type, etc.), these models will always necessarily be incomplete. At present, only the models for snow crab have been completed using this method (Figure~\ref{fig:snowcrabHabitat}; see \cite{snowcrabResDoc} for more details). Further, it is possible to model the spatial/temporal patterns (Section~\ref{sec:interpolation}) and then "integrate" them in a risk-based approach (Section~\ref{sec:riskapproach}) to permit more formal statements of risk and probability of exceeding thresholds. 
...
\begin{figure}[h]
  ...
  \caption{Predicted probability of observing snow crab in Maritimes Region based upon a Deconstructive-habitat method.}
\end{figure}
...
While the Deconstructive habitat concept is interesting and useful when a reductionistic understanding is imperative, the monitoring and assessment requirements for an MPA  demands also a whole system understanding. The hypothesis put forward is that the relative abundance of organisms found in a given location and time defines and mirrors the kind of habitat in which they live. Sessile organisms that require high flow environments and associated biota tend to be of one groups of organisms; and they are different from those that require cold waters and minimal water flow, etc. Thus if we can quantify the observed species composition in a given location and time, we would, in effect, be describing the habitat. That is species assemblage information can reflect all these biological and environmental interactions, measured and unmeasured. These associations are readily quantified using a standard multivariate procedures. Here we focus upon, Principal Components Analysis which focuses upon the correlational structure of the species assemblages and Correspondence Analysis which focuses upon chi-squared differences (Figure~\ref{fig:speciesCompositionMap}). We will define this as \textbf{Integral-habitat}. 

The intent is to develop spatio-temporal models of Integral-habitat and then similarly integrate and model them in a risk-based approach to permit more formal statements of risk and probability of exceeding thresholds  (Section~\ref{sec:riskapproach}).
