# R script for extacting bathymetric data from CHS DEM files on BANK-MFLIB


get.bathy <- function(area='custom',xl,yl){

# Function for connecting to ____ and
# Extracting subset of bathymetric data from _______mflib.chsdem_ind________
...
#
	...
	chan <- odbcConnect(oracle.clam.dsn, oracle.clam.user, oracle.clam.password)
...
if(area=='BANQ')	qu.bathy <- paste("SELECT x,(to_number(substr(x,1,2)) + to_number(substr(x,4,2))/60. + to_number(substr(x,7,6))/3600.) * decode(substr(x,13,1),'S',-1,1) lat,    y,   (to_number(substr(y,1,2)) + to_number(substr(y,4,2))/60. + to_number(substr(y,7,6))/3600.) * decode(substr(y,13,1),'W',-1,1) lon,    z FROM mflib.chsdem_ind WHERE x between '44-00-00.000N' and '45-15-00.000N' and y between '57-00-00.000W' and '60-05-00.000W';")
