...
marport.db = function( DS, p ){
  ...
  if(DS %in% c("marport", "marport.redo"))  {
    basedata=NULL
    fn=file.path( p$marport.dir, paste( "marport", "rdata", sep="." ))
    if(DS == "marport"){
    ...
    filelist4 = list.files(path=p$marport.dir, pattern="^config.*.ini$", full.names=T, recursive=TRUE, ignore.case=TRUE)
    cfg = data.frame( configroot = dirname( filelist4 ), fn=filelist4, stringsAsFactors=FALSE )
    
    filelist1 = list.files(path=p$marport.dir, pattern=".log$", full.names=T, recursive=TRUE, ignore.case=TRUE)
    filelist2 = list.files(path=p$marport.dir, pattern=".gps$", full.names=T, recursive=TRUE, ignore.case=TRUE)
    filelist3 = list.files(path=p$marport.dir, pattern=".sgp$", full.names=T, recursive=TRUE, ignore.case=TRUE)
