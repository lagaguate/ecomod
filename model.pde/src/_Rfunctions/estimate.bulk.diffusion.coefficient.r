  ...
  estimate.bulk.diffusion.coefficient = function( DS="snowcrab" ) { 
    ...
    if (DS=="snowcrab") {
      ...
      # advection and diffusion approach 
      DC = list()
      ...
      for ( doff in 1:(length(times)-1) ) {
        ...
        if (length(ss)>3 ) {
          vel = lm ( dt ~ dx, data=move[ss,], na.action=na.omit )
          move$dexpected[ss] = predict( vel, move[ss,] ) 
          DC[[ doff ]] = lm( dt*2 ~ (dx-dexpected)^2 -1, data=move[ss,], na.action=na.omit ) 
        }
      }   
      lapply(DC, summary)
      ...
      odc = matrix( unlist(lapply(DC, coef)), ncol=1, byrow=T)
      odc = cbind( times[-1], odc )
      ...
      summary(move$velocity[is.finite( move$velocity)])
      mean ( move$velocity[is.finite( move$velocity)])
      median ( move$velocity[is.finite( move$velocity)])
      var ( move$velocity[is.finite( move$velocity)])
