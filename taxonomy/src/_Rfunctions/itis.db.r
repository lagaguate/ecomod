  ...
  itis.db = function( DS="itaxa", lnk=NULL, itis.kingdom=NULL ) {
    ...
    if ( DS %in% c( "main.redo", "taxon.unit.types", "kingdoms", "itaxa" ) ) {
      ...
      taxonomic.units = read.table( gzfile( file.path( itis.dir, "taxonomic_units.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      geographic.div = read.table( gzfile( file.path( itis.dir, "geographic_div.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      jurisdiction = read.table( gzfile( file.path( itis.dir, "jurisdiction.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      longnames = read.table( gzfile( file.path( itis.dir, "longnames.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      synonym.links = read.table( gzfile( file.path( itis.dir, "synonym_links.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      vernaculars =  read.table( gzfile( file.path( itis.dir, "vernaculars.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      kingdoms = read.table( gzfile( file.path( itis.dir, "kingdoms.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      names( kingdoms ) = c( "kingdom_id", "kingdom_name", "update_date" )
      # primary keys = kingdom_id
      kingdoms = kingdoms[ , c( "kingdom_id", "kingdom_name" ) ]
 

      taxon.unit.types = read.table( gzfile( file.path( itis.dir, "taxon_unit_types.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
      ...
      ### the rest are unused
        ...
        if (! just.what.is.needed) {
          ...
          hierachy = read.table( gzfile( file.path( itis.dir, "hierarchy.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
          ...
          comments = read.table( gzfile( file.path( itis.dir, "comments.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
          names( comments ) = c( "comment_id", "commentator", "comment_detail", "comment_time_stamp", "update_date" )
          # primary keys = comment_id

          taxa_comments_links = read.table( gzfile( file.path( itis.dir, "tu_comments_links.gz" ) ), sep="|", stringsAsFactors=F, quote="", fill=T )
