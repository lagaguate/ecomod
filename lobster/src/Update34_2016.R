...
## Landings
	...
	Seasonal34<-subset(seasonal.landings,YR<2016,c("SYEAR","LFA34","YR"))
...
qq		with(Annual34[which(Annual34$YR<p$current.assessment.year),],plot(YR,LFA34,xlab='Year',ylab='Landings (t)',type='h',lwd=4, ylim=c(2000,max(c(Seasonal34$LFA34,Annual34$LFA34)))))
		with(Seasonal34[which(Seasonal34$YR<p$current.assessment.year),],lines(YR,LFA34,col='red',lwd=2))
	...
	dev.off()
		...
		#3-year running mean
		apply(embed(xpp$mean,running.length),1,mean)

		apply(embed(Seasonal34$LFA34,3),1,mean)


#Beginning of a stock status plot
	with(subset(b,YR<2015),plot(LFA34/USR,NUM_OF_TRAPS/1E6,type='b',xlim=c(0,3),ylim=c(16,21)))
	abline(v=1,col='red')
	polygon(c(0,1,1,0),c(0,0,25,25),col='yellow')
	polygon(c(1,6,6,1),c(0,0,25,25),col='green')
	with(subset(b,YR<2015),lines(LFA34/USR,NUM_OF_TRAPS/1E6,type='b',xlim=c(0,3),ylim=c(16,21)))
	...
	LFA34logData<-subset(LFA34logData,!="OOS"&SYEAR<2015)
