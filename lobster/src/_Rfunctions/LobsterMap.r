...
options(stringsAsFactors=F)		
	...
	if(!is.null(nafo)){
		...
		addPolys(nafo.xy,border='grey',col=NA)
...
#groundfish survey summer strata	
	if(addSummerStrata) {
			  ...
			  addPolys(b,lty=1,border='black',col=NA)
			 # addLabels(a,cex=0.6)
			}
  # Boundries
	...
	if(boundaries=='LFAs'){
		...
		if(lfa%in%LFAgrid$PID){
			if(!is.na(lfa)){
				...
				addPolys(grids,border=rgb(0,0,0,0.2),col=NA)
				if(labels=='grid'){
				...
				addPolys(LFAgrid,border=rgb(0,0,0,0.2),col=NA)
			}
		}
		#browser()
		addPolys(LFAs)
