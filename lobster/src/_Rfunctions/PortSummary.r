PortSummary<-function(lfa='34',bins=seq(50,200,5),output='graph'){
	...
	logs<-subset(read.csv(file.path( project.datadirectory("lobster"), "data","logsInSeason.csv")),LFA==lfa)
	allPorts<-subset(read.csv(file.path( project.datadirectory("lobster"), "data","Ports.csv")),LFA==lfa)
	...
	if(output=='graph'){
		...
		for(i in 1:length(ports)){
			...
			pdf(file.path(project.datadirectory('lobster'),"R","PortSummary",paste0("PortSummary",ports[i],".pdf")))
			#browser()
