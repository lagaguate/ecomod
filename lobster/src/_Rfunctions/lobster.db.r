  ...
  lobster.db = function( DS="", p="" ) {
    ...
    if (DS %in% c("groundfish", "groundfish.redo") ) {
      ...
      i=which(duplicated( sm$id ))
      if (length(i) >0) sm = sm[ -i ,]

      set = groundfish.db( "set" )
      set = set[, c("spec", "id", "totwgt", "totno") ]
      set = set[ which( set$spec %in% p$taxa.codes ) ,]
      i=which(duplicated( set$id ))
      set = set[ -i ,]
      gf = merge(sm, set, by="id", sort=F, all.x=T, all.y=F)
      gf$PA = 0
    ...
    if (DS %in% c("complete", "complete.redo") ) {
      ...
      tokeep = c( "id", "source", "lon", "lat",  "chron", "yr", "julian", "z", "t", "sal",
        ...
        "tamp", "wmin", "thp", "tsd.annual", "tmean.annual", "tamp.annual", "wmin.annual", "tsd.annual" ) 
