PortSamplesProcess<-function(lfa='34',min.size=80){
	...
	lobster.db('port')
	port1<-subset(port,LFA==lfa&L_SIZE>min.size)
	port1$SYEAR<-as.numeric(substr(port1$SEASON,6,9))
	port1$SAMPLE_ID<-as.numeric(paste(port1$SAMPLE_SEQ,port1$SAMPLE_NO,sep='.'))
	
	# individual samples
	column.select<-c("SAMPLE_ID","SAMPLE_SEQ","SAMPLE_NO", "SDATE","SEASON","SYEAR", "NTRAPS", "LATITUDE", "LONGITUDE","LFA","PORT", "COUNTY", "STAT","GRID_NUM", "PORT_CODE","PORT_LAT", "PORT_LON")
	...
	portlengths<-merge(portsamples[,c("SAMPLE_ID","SYEAR","Q")],portlengths,all=T)[,c("SAMPLE_ID","SYEAR","LENGTH","Q","SEX")]
