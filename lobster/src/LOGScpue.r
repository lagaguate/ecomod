...
lobLogs$TOTAL_NUM_TRAPS<-rowSums(lobLogs[c('NUM_OF_TRAPS','NUM_OF_TRAPS_B','NUM_OF_TRAPS_C')],na.rm=T)
lobLogs$TOTAL_WEIGHT_LBS<-rowSums(lobLogs[c('WEIGHT_LBS','WEIGHT_LBS_B','WEIGHT_LBS_C')],na.rm=T)
lobLogs$TOTAL_WEIGHT_KG<-lobLogs$TOTAL_WEIGHT_LBS*0.4536

# select for records within season
lobLogs$SYEAR<-NA
lobLogs$DATE_FISHED<-as.Date(lobLogs$DATE_FISHED)
for(i in 1:length(lfa)) {
	h <- Fish.Date[Fish.Date$LFA==lfa[i],]	
	for(j in 1:nrow(h)) {
		lobLogs$SYEAR[lobLogs$LFA==lfa[i]&lobLogs$DATE_FISHED>=h[j,'START_DATE']&lobLogs$DATE_FISHED<=h[j,'END_DATE']]<-h[j,'SYEAR']
	}

}
lobLogs<-subset(lobLogs,!is.na(SYEAR))

# add week of season (WOS) variable
lobLogs$WOS<-NA
for(i in 1:length(lfa)) {
	h <- Fish.Date[Fish.Date$LFA==lfa[i],]	
	for(j in unique(lobLogs$SYEAR[lobLogs$LFA==lfa[i]])){
		lobLogs$WOS[lobLogs$LFA==lfa[i]&lobLogs$SYEAR==j]<-floor(as.numeric(lobLogs$DATE_FISHED[lobLogs$LFA==lfa[i]&lobLogs$SYEAR==j]-min(h$START_DATE[h$SYEAR==j]))/7)+1
	}
}
...
lobLogsInSeasonA<-subset(lobLogs,!is.na(SYEAR)&!is.na(WEIGHT_LBS)&!is.na(NUM_OF_TRAPS),c(commonCols,"GRID_NUM", "WEIGHT_LBS", "NUM_OF_TRAPS"))
lobLogsInSeasonB<-subset(lobLogs,!is.na(SYEAR)&!is.na(WEIGHT_LBS_B)&!is.na(NUM_OF_TRAPS_B),c(commonCols,"GRID_NUM_B", "WEIGHT_LBS_B", "NUM_OF_TRAPS_B"))
lobLogsInSeasonC<-subset(lobLogs,!is.na(SYEAR)&!is.na(WEIGHT_LBS_C)&!is.na(NUM_OF_TRAPS_C),c(commonCols,"GRID_NUM_C", "WEIGHT_LBS_C", "NUM_OF_TRAPS_C"))
names(lobLogsInSeasonB)<-names(lobLogsInSeasonA)
names(lobLogsInSeasonC)<-names(lobLogsInSeasonA)
lobLogsInSeason<-rbind(lobLogsInSeasonA,lobLogsInSeasonB,lobLogsInSeasonC)
lobLogsInSeason$WEIGHT_KG<-lobLogsInSeason$WEIGHT_LBS*0.4536
...
lobLogsInSeason<-subset(lobLogsInSeason,!is.na(GRID_NUM)&paste(LFA,GRID_NUM,sep='.')%in%grid.key)
lobLogsInSeason$CPUE<-lobLogsInSeason$WEIGHT_KG/lobLogsInSeason$NUM_OF_TRAPS
lobLogsInSeason<-subset(lobLogsInSeason,CPUE<20)

subareas<-read.csv(file.path( project.datadirectory("lobster"), "data","LFA2733subarea.csv"))
names(subareas)[2]<-"GRID_NUM"
lobLogsInSeason<-merge(lobLogsInSeason,subareas,all.x=T)

cpue.lst<-list()
cpue1.lst<-list()
for(i in 1:length(lfa)) {
	dat<-subset(lobLogsInSeason,LFA==lfa[i]&TOTAL_NUM_TRAPS<max_trap[i],c('subarea','SYEAR','WEIGHT_KG','NUM_OF_TRAPS'))
	if(lfa[i]%in%c('27','33')){
...
for(i in 1:length(lfa)) {
	for(j in 2008:2014){
		...
		par(mfrow=c(3,2))
		d<-subset(lobLogsInSeason,LFA==lfa[i]&SYEAR==j)
...
aggDat99<-aggregate(CPUE ~ LFA + SYEAR + WOS, data = lobLogsInSeason, quantile,0.99)
aggDat999<-aggregate(CPUE ~ LFA + SYEAR + WOS, data = lobLogsInSeason, quantile,0.999)
par(mfcol=c(3,2))
