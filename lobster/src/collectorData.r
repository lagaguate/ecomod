	...
	xyplot(Collector.Count~as.numeric(Year)|factor(Study.Area),data=a, main="N Collectors Sampled by Year", xlab='Year',type='h',lwd=5,xlim=c(2006,2016))
...
require(mixtools)
	...
	hist(yy,breaks = seq(1,100,2),main=s)
	sC = normalmixEM(yy,k=3, mu=c(8,22,38),sigma=c(0.5,1,3) )
	histAllMixtures(sC,breaks=seq(1,100,2),main=s,probability=T)	
