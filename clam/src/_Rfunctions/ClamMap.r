...
ClamMap<-function(area = 'custom', ylim = c(41,47), xlim = c(-68, -57), title = '',
  ...
  if(isobath[1]=='quick'){
    ...
    if (as.numeric(xlim[2]) < -50.){        ## Scotian Shelf
      d50.ll<-read.table("common/data/spatial/bathymetry/d50.ll", header=T)
      d50.ll<-na.omit(d50.ll)
      d100.ll<-read.table("common/data/spatial/bathymetry/d100.ll", header=T)
      d100.ll<-na.omit(d100.ll)
      d150.ll<-read.table("common/data/spatial/bathymetry/d150.ll", header=T)
      d150.ll<-na.omit(d150.ll)
      d200.ll<-read.table("common/data/spatial/bathymetry/d200.ll", header=T)
      ...
      chs50.ll<-read.table("common/data/spatial/bathymetry/CHS50.ll", header=T)
      chs50.ll<-na.omit(chs50.ll)
      chs100.ll<-read.table("common/data/spatial/bathymetry/CHS100.ll", header=T)
      chs100.ll<-na.omit(chs100.ll)
      chs200.ll<-read.table("common/data/spatial/bathymetry/CHS200.ll", header=T)
  ...
  if(!is.null(nafo)){
    ...
    nafo.xy <- read.csv("common/data/spatial/labels/nafo.csv")
  ...
  if(banks){
    ...
    bankNames.xy <- read.csv("common/data/spatial/labels/bankNames.csv")
