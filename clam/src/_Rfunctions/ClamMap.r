...
ClamMap<-function(area = 'custom', ylim = c(41,47), xlim = c(-68, -57), title = '',
	...
	if(res=='high')land <- read.table("F:/Dale/R_Maps/martimesHIGH.ll", header=T)
	...
	if(!is.null(image.lst)){
		...
		if(image.lst=='bathymetry'){
			source("F:/Dale/R_Maps/get.bathy.r", local=T)
...
#			bathy.lst <- makeTopography(bathy.dat)
	...
	if(isobath[1]=='quick'){
		d50.ll<-read.table("F:/Dale/R_Maps/d50.ll", header=T)
		d50.ll<-na.omit(d50.ll)
		d100.ll<-read.table("F:/Dale/R_Maps/d100.ll", header=T)
		d100.ll<-na.omit(d100.ll)
		d150.ll<-read.table("F:/Dale/R_Maps/d150.ll", header=T)
		d150.ll<-na.omit(d150.ll)
		d200.ll<-read.table("F:/Dale/R_Maps/d200.ll", header=T)
	...
	if(!is.null(nafo)){
        ...
        nafo.xy <- read.csv("F:/Dale/R_Maps/nafo.csv")
	...
	if(banks){
			 ...
			 bankNames.xy <- read.csv("F:/Dale/R_Maps/bankNames.csv")
