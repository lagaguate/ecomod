...
#--      set = set[ which( set$id %in% unique( c(unique( det$id), unique(cat$id))) ),]
      ...
      # create a model for length-weight each {species, sex} and then use it to predict and estimate residuals
      ...
      detmr = condition.estimate ?? ( det$mass, det$t, tK=10 )
      det = cbind( det, detmr )
    ...
    if (DS %in% c( "condition.filtered", "condition.filtered.redo" ) ) {
 
      ddir = file.path( project.directory("condition"), "data", p$spatial.domain, p$season )
      dir.create( ddir, showWarnings=FALSE, recursive=TRUE )
      
      fn = file.path( ddir, "set.condition.filtered.rdata" )
        
      if (DS=="condition.filtered") {
        MR = NULL
        if (file.exists( fn) ) load( fn ) 
        return ( MR )
      }
      
      MR =  condition.db( DS="condition", p=p )
...
#         ulsmrA  =10^-2; MR$smrA  [ which(MR$smrA   > ulsmrA )]   = ulsmrA
    ...
    if (DS %in% c( "condition.merged", "condition.merged.redo" ) ) {
      ...
      mm =  condition.db( DS="condition.filtered", p=p )
