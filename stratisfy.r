...
get.stratum.tables<-function(agency.gui){
  if (agency.gui=="NMFS"){
    stratum.tables<-c("USNEFSC.DFO5ZJM", "USNEFSC.DFO5ZGHNO", 
                      "USNEFSC.NMFS5ZJM", "USNEFSC.NMFS5ZGHNO", 
                      "USNEFSC.NMFS5ZJMC", "USNEFSC.NMFS5ZJMU", 
                      "USNEFSC.NMFS5ZU")
  }
...
get.species<-function(agency.gui, by.sex.gui) {
   ...
   #had species with more than 1 catchsex, and manually paste them into
    if (agency.gui=="NMFS"){
      ...
      species.query=paste("SELECT SPEC, INITCAP(CNAME), 
                            LGRP, LFSEXED FROM USNEFSC.USSPEC 
                            WHERE  SPEC <> 9999 "
                            ,species.query.tweak,
      ...
      species.query=paste("SELECT DISTINCT(SPEC), initcap(CNAME), 
                            LGRP, LFSEXED 
...
get.missions<-function(agency.gui,year.gui){
  if (agency.gui=="NMFS"){
    mission.query=paste0("SELECT CRUISE6 AS MISSION, SVVESSEL AS VESEL, 
...
species.gui<-select.list(get.species(agency.gui, by.sex.gui),
                         multiple=F, graphics=T, 
...
missions.gui<-SQL.in(mission.df$MISSION)
...
input_parameters<-list("Stratisfy version"=paste(
  stratisfy.ver, "(",mode,")"
  , sep=""),
