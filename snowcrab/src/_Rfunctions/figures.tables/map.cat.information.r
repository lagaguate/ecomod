  ...
  map.cat.information = function(p, outdir ) {
    ...
    for (sp in species) {
      ...
      sset = bcat[ filter.taxa (x=bcat$spec, method=sp), outvars]
      if (dim(sset)[1] > 1) {
        variables = c("totno")
        ...
        make.maps( sset, p, variables=variables, plottimes=p$plottimes, basedir=basedir, conversions=p$conversions, init.files=p$env.init )
       }
    }
    return( "Done" )
  }
